...
#pragma warning disable 1591
        ...
        internal const string IDX13102 =
            "IDX13102: Exception thrown while reading '{0}' for Saml2SecurityToken. Inner exception: '{1}'.";

        internal const string IDX13106 =
            "IDX13106: Unable to read for Saml2SecurityToken. Element: '{0}' as missing Attribute: '{1}'.";
        ...
        internal const string IDX13137 =
            "IDX13137: Unable to read for Saml2SecurityToken. Version must be '2.0' was: '{0}'.";
        ...
        internal const string IDX13303 =
            "IDX13303: 'Subject' is required in Saml2Assertion for built-in statement type.";
    ...
    class Saml2PSerializer : Saml2Serializer
    {
        ...
        public override Saml2Assertion ReadAssertion(XmlReader reader)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(version))
                    throw LogReadException(LogMessages.IDX13106, Saml2Constants.Elements.Assertion,
                ...
                assertion.IssueInstant = DateTime.ParseExact(value, Saml2Constants.AcceptedDateTimeFormats,
                    DateTimeFormatInfo.InvariantInfo, DateTimeStyles.None).ToUniversalTime();
                ...
                while (envelopeReader.IsStartElement())
                {
                    ...
                    else if (envelopeReader.IsStartElement(Saml2Constants.Elements.AttributeStatement,
                        Saml2Constants.Namespace))
                        statement = ReadAttributeStatement(envelopeReader);
                    else if (envelopeReader.IsStartElement(Saml2Constants.Elements.AuthnStatement,
                        Saml2Constants.Namespace))
                        statement = ReadAuthenticationStatement(envelopeReader);
                    else if (envelopeReader.IsStartElement(Saml2Constants.Elements.AuthzDecisionStatement,
                        Saml2Constants.Namespace))
        ...
        public override void WriteAssertion(XmlWriter writer, Saml2Assertion assertion)
        {
            ...
            if (assertion.SigningCredentials != null)
                writer = signatureWriter = new EnvelopedSignatureWriterWithReferenceIdFix(writer,
                        assertion.SigningCredentials, assertion.Id.Value, assertion.InclusiveNamespacesPrefixList)
        ...
        protected override Saml2AuthenticationContext ReadAuthenticationContext(XmlDictionaryReader reader)
        {
            if (spOptions?.Compatibility?.IgnoreAuthenticationContextInResponse ?? false)
            {
                ...
                return (Saml2AuthenticationContext) System.Runtime.Serialization.FormatterServices
                    .GetUninitializedObject(typeof(Saml2AuthenticationContext));
            }
