...
namespace Sustainsys.Saml2.Saml2P
{
    ...
    public class EnvelopedSignatureWriterWithReferenceIdFix : DelegatingXmlDictionaryWriter
    {
        ...
        public EnvelopedSignatureWriterWithReferenceIdFix(XmlWriter writer, SigningCredentials signingCredentials,
            string referenceId)
        ...
        /// <exception cref="ArgumentNullException">if <paramref name="referenceId"/> is null or Empty.</exception>
        public EnvelopedSignatureWriterWithReferenceIdFix(XmlWriter writer, SigningCredentials signingCredentials,
            string referenceId, string inclusivePrefixList)
        {
        ...
        private void OnEndRootElement()
        {
            ...
            Array.Copy(writerBytes, (int) _signaturePosition, effectiveBytes,
                (int) _signaturePosition + signatureBytes.Length, writerBytes.Length - (int) _signaturePosition);
        ...
        private Signature CreateSignature()
        {
            CryptoProviderFactory cryptoProviderFactory =
                _signingCredentials.CryptoProviderFactory ?? _signingCredentials.Key.CryptoProviderFactory;
            var hashAlgorithm = cryptoProviderFactory.CreateHashAlgorithm(_signingCredentials.Digest);
            if (hashAlgorithm == null)
                throw LogExceptionMessage(new XmlValidationException(FormatInvariant(LogMessages.IDX30213,
                    cryptoProviderFactory.ToString(), _signingCredentials.Digest)));

            Reference reference = null;
            try
            {
                reference = new Reference(new EnvelopedSignatureTransform(),
                    new ExclusiveCanonicalizationTransform
                        {InclusiveNamespacesPrefixList = _inclusiveNamespacesPrefixList})
                {
                    Uri = "#" + _referenceId,
            ...
            var provider =
                cryptoProviderFactory.CreateForSigning(_signingCredentials.Key, _signingCredentials.Algorithm);
            if (provider == null)
                throw LogExceptionMessage(new XmlValidationException(FormatInvariant(LogMessages.IDX30213,
                    cryptoProviderFactory.ToString(), _signingCredentials.Key.ToString(),
