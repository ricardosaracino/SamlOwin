...
namespace Sustainsys.Saml2
{
    ...
    public static class XmlHelpers
    {
        ...
        internal static string GetRequiredAttributeValue(this XmlElement node, string attributeName)
        {
            ...
            if (string.IsNullOrWhiteSpace(foundAttribute?.Value))
            {
                throw new BadFormatSamlResponseException(
                    $"Attribute '{attributeName}' (case-sensitive) was not found or its value is empty");
            }
        ...
        internal static XmlElement GetRequiredElement(this XmlElement node, string name, string namespaceValue)
        {
            ...
            {
                throw new BadFormatSamlResponseException(
                    $"Element '{name}' (case-sensitive, namespace '{namespaceValue}') was not found");
            }
        ...
        [SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope", Justification =
            "I don't care, the StringWriter contains no references to unmanaged resources")]
